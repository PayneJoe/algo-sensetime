...
object ParallelizedLR {
  ...
  */
  def evaluate(data: RDD[(Long,LabeledPoint)], model: BDV[Double],metric: (String,String),lossType: String): (Double,Double) = {
    ...
    val ret1 = metric._1 match{
      case "accuracy" => Metrics.computeAccuracy(y_truth,y_predict,lossType)
      case "loss" => Metrics.computeLoss(y_truth,y_predict,lossType)
    }
    val ret2 = metric._2 match{
      case "auc" => Metrics.computeAuc(y_truth,y_predict,lossType)
      case _ => 0.0
    }
  ...
  def main(args: Array[String]): Unit ={
    ...
    method match{
      case "sgd" => trainLRWithSGD(train,validate,iterNum,alpha,lambda,metricType,regularType,lossType)
      case "lbfgs" => trainLRWithLBFGS(train, validate, iterNum, alpha, lambda, metricType, regularType, lossType)
    }
  }
}
